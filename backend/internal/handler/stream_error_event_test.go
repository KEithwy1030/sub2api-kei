...
func parseResponsesFailedSSE(t *testing.T, body string) (map[string]any, map[string]any) {
	...
	sequenceNumber, hasSeq := parsed["sequence_number"].(float64)
	require.True(t, hasSeq, "strict Responses clients require sequence_number")
	assert.Equal(t, float64(0), sequenceNumber)
	...
	createdAt, ok := resp["created_at"].(float64)
	require.True(t, ok, "created_at must be present for strict Responses clients")
