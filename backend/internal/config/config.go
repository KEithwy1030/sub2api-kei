...
type GatewayConfig struct {
	...
	// GrokFirstOutputTimeoutSeconds: Grok Responses 首个语义输出超时（秒），0表示禁用。
	GrokFirstOutputTimeoutSeconds int `mapstructure:"grok_first_output_timeout_seconds"`
	// GrokHighEffortFirstOutputTimeoutSeconds: Grok high/xhigh/max 推理的首个语义输出超时（秒）。
	// 0 表示回退到 GrokFirstOutputTimeoutSeconds。
	GrokHighEffortFirstOutputTimeoutSeconds int `mapstructure:"grok_high_effort_first_output_timeout_seconds"`
...
func setDefaults() {
	...
	viper.SetDefault("gateway.grok_first_output_timeout_seconds", 0)
	viper.SetDefault("gateway.grok_high_effort_first_output_timeout_seconds", 0)
...
func (c *Config) Validate() error {
	...
	if c.Gateway.GrokFirstOutputTimeoutSeconds < 0 || c.Gateway.GrokFirstOutputTimeoutSeconds > 600 ||
		(c.Gateway.GrokFirstOutputTimeoutSeconds > 0 && c.Gateway.GrokFirstOutputTimeoutSeconds < 30) {
		return fmt.Errorf("gateway.grok_first_output_timeout_seconds must be 0 or between 30-600 seconds")
	}
	if c.Gateway.GrokHighEffortFirstOutputTimeoutSeconds < 0 || c.Gateway.GrokHighEffortFirstOutputTimeoutSeconds > 1800 ||
		(c.Gateway.GrokHighEffortFirstOutputTimeoutSeconds > 0 && c.Gateway.GrokHighEffortFirstOutputTimeoutSeconds < 30) {
		return fmt.Errorf("gateway.grok_high_effort_first_output_timeout_seconds must be 0 or between 30-1800 seconds")
	}
