...
func normalizeOpenAIPassthroughOAuthBody(body []byte, compact bool) ([]byte, bool, error) {
	...
	// ChatGPT OAuth responses rejects several tuning/output fields that some
	// clients still send. Mirror the legacy Codex transform behavior here so
	// passthrough remains compatible with existing OpenClaw/OpenAI callers.
	for _, key := range []string{
		...
		if field := gjson.GetBytes(normalized, key); field.Exists() {
			next, err := sjson.DeleteBytes(normalized, key)
			if err != nil {
				return body, false, fmt.Errorf("normalize passthrough body delete %s: %w", key, err)
			}
			normalized = next
			changed = true
		}
	...
	inputNormalized, inputChanged, err := normalizeCodexJSONInputField(normalized)
	if err != nil {
	...
	instructionsNormalized, instructionsChanged, err := normalizeOpenAIPassthroughInstructionsField(normalized)
	if err != nil {
		return body, false, err
	}
	if instructionsChanged {
		normalized = instructionsNormalized
		changed = true
	}
...
// normalizeCodexJSONInputField ensures OAuth Codex requests always carry list-shaped input.
func normalizeCodexJSONInputField(body []byte) ([]byte, bool, error) {
	if len(body) == 0 {
		return body, false, nil
	}
	...
	case gjson.String:
		...
		if strings.TrimSpace(text) == "" {
			next, err := sjson.SetRawBytes(body, "input", []byte("[]"))
			if err != nil {
				return body, false, fmt.Errorf("normalize passthrough body input empty string: %w", err)
			}
		...
		if err != nil {
			return body, false, fmt.Errorf("normalize passthrough body input string marshal: %w", err)
		}
		next, err := sjson.SetRawBytes(body, "input", payload)
		if err != nil {
	...
	case gjson.JSON:
		...
		if strings.HasPrefix(raw, "{") {
			next, err := sjson.SetRawBytes(body, "input", []byte("["+raw+"]"))
			if err != nil {
	...
	case gjson.Null:
		next, err := sjson.SetRawBytes(body, "input", []byte("[]"))
		if err != nil {
			return body, false, fmt.Errorf("normalize passthrough body input null: %w", err)
		}
		return next, true, nil
	default:
		return body, false, nil
...
func normalizeOpenAIPassthroughInstructionsField(body []byte) ([]byte, bool, error) {
	if len(body) == 0 {
		return body, false, nil
	}

	input := gjson.GetBytes(body, "input")
	if !input.Exists() || input.Type != gjson.JSON {
		return body, false, nil
	}
	...
	if err := json.Unmarshal([]byte(raw), &items); err != nil {
		return body, false, fmt.Errorf("normalize passthrough body input array: %w", err)
	}

	developerTexts := make([]string, 0, 2)
	...
	for _, item := range items {
		...
		text := strings.TrimSpace(extractOpenAIInputText(record["content"]))
		if text == "" {
			filtered = append(filtered, item)
			continue
		}
	...
	instructionsParts := make([]string, 0, len(developerTexts)+1)
	if existing := strings.TrimSpace(gjson.GetBytes(body, "instructions").String()); existing != "" {
		instructionsParts = append(instructionsParts, existing)
	}
	instructionsParts = append(instructionsParts, developerTexts...)
	combinedInstructions := strings.Join(instructionsParts, "\n\n")

	next, err := sjson.SetBytes(body, "instructions", combinedInstructions)
	if err != nil {
	...
	if err != nil {
		return body, false, fmt.Errorf("normalize passthrough body filtered input marshal: %w", err)
	}
	next, err = sjson.SetRawBytes(next, "input", filteredPayload)
	if err != nil {
		return body, false, fmt.Errorf("normalize passthrough body filtered input: %w", err)
	}

	return next, true, nil
}

func extractOpenAIInputText(content any) string {
	...
	case []any:
		...
		for _, item := range typed {
			...
			partType := strings.ToLower(strings.TrimSpace(stringValue(record["type"])))
			switch partType {
			case "input_text", "text", "output_text":
				text := strings.TrimSpace(stringValue(record["text"]))
				if text != "" {
