...
func applyCodexOAuthTransform(reqBody map[string]any, isCodexCLI bool, isCompact bool) codexTransformResult {
	...
	if input, ok := reqBody["input"].([]any); ok {
		...
		// Some clients send a single message object. Codex endpoint expects a list.
		reqBody["input"] = []any{inputMap}
		result.Modified = true
	} else if inputSlice, ok := coerceAnySlice(reqBody["input"]); ok {
		inputSlice = filterCodexInput(inputSlice, needsToolContinuation)
		reqBody["input"] = inputSlice
		result.Modified = true
	} else if inputValue, exists := reqBody["input"]; exists && inputValue == nil {
		reqBody["input"] = []any{}
		result.Modified = true
	}
