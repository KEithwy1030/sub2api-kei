...
import { parseGrokSSOInput } from '@/api/admin/grok'
...
const handleGrokImportSSO = async (ssoInput: string) => {
  // Full grok-register lines are parsed locally so email/password never leave the browser.
  const ssoTokens = parseGrokSSOInput(ssoInput)
